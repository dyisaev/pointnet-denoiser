...
from .nn_utils import (
    BatchNorm1D_controlled_bias,
    count_parameters,
    count_parameters_requiring_grad,
)
...
class PointNet_Denoiser(nn.Module):
    ...
    def __init__(self, point_dim=3, use_bias=True):
        ...
        self.decoder = PointNet_Decoder(
            input_dim=point_dim + self.encoder.output_shape[0],
            point_dim=point_dim,
            use_bias=use_bias,
        )
...
if __name__ == "__main__":
    ...
    print(
        "Denoiser parameters requiring grad:", count_parameters_requiring_grad(denoiser)
    )
